...
public abstract class Configuration {
    ...
    public static final String DAEMON_LOGS_DIR;
    ...
    static {
        ...
        if (USER_HOME != null && kHome == null) {
            System.setProperty("kompics.home", new File(USER_HOME + "/.kompics/").getAbsolutePath());
        ...
        DAEMON_LOGS_DIR = KOMPICS_HOME + "/daemon_logs/";
        
        if (new File(Configuration.KOMPICS_HOME).exists() == false) {
            if (new File(Configuration.KOMPICS_HOME).mkdirs() == false) {
                logger.warn("Could not create directory: " + Configuration.KOMPICS_HOME);
            }
        }

        String mavenHome = System.getProperty("maven.repo.local");
        if (mavenHome == null) {
            System.setProperty("maven.repo.local", new File(USER_HOME + "/.m2/repository").getAbsolutePath());
        }
        ...
        if (new File(Configuration.MAVEN_REPO_LOCAL).exists() == false) {
            if ((new File(Configuration.MAVEN_REPO_LOCAL).mkdirs()) == false) {
                logger.warn("Couldn't set directory for Maven Local Repository: " + Configuration.MAVEN_REPO_LOCAL + "\nCheck file permissions for this directory.");
            }
        }
    ...
    public static final int DEFAULT_MASTER_PORT = 2324;
    public static final int DEFAULT_DAEMON_PORT = 2322;
    public static final int DEFAULT_PEER_PORT = 2325;
    public static final int DEFAULT_MASTER_ID = 0;
    public static final int DEFAULT_DAEMON_ID = 1;
    public static final int DEFAULT_PEER_ID = 10;
    public static final String DEFAULT_BOOTSTRAP_IP = LocalNetworkConfiguration.findLocalHostAddress();
    public static final int DEFAULT_BOOTSTRAP_PORT = 20002;
    public static final int DEFAULT_BOOTSTRAP_ID = Integer.MAX_VALUE;
    public static final String DEFAULT_MONITOR_IP = LocalNetworkConfiguration.findLocalHostAddress();
...
//    protected P2pMonitorConfiguration monitorConfiguration = null;
    ...
    public static final synchronized Configuration init(String[] args,
        ...
        try {
            ...
            Constructor<? extends Configuration> constructor = classname[0].getConstructor(String[].class);
    ...
    protected Configuration(String[] args) throws IOException {
        ...
        try {
            ...
            logger.warn("Bootstrap configuration file not found, using default values: " + BOOTSTRAP_CONFIG_FILE);
        }
        ...
        try {
            ...
            logger.warn("Monitor configuration file not found, using default values: " + MONITOR_CONFIG_FILE);
        }
        ...
        try {
            ...
            logger.warn("Experiment configuration file not found, using default values: " + EXPERIMENT_CONFIG_FILE);
        }
        ...
        webAddress = "http://" + compositeConfig.getString(
                PROP_IP, DEFAULT_IP) + ":" + webPort + "/";

        bootConfiguration = setupBootstrapConfig();

//        monitorConfiguration = setupMonitorConfig();

        homepage = "<h2>Welcome to the Kompics Peer-to-Peer Framework!</h2>" + "<a href=\"" + webAddress + bootConfiguration.getBootstrapServerAddress().getId() + "/" + "\">Bootstrap Server</a><br>";
//     + "<a href=\"" + webAddress
//                + monitorConfiguration.getMonitorServerAddress().getId() + "/" + "\">Monitor Server</a><br>";
    ...
    private BootstrapConfiguration setupBootstrapConfig() throws UnknownHostException {
        ...
        return new BootstrapConfiguration(bootstrapAddress, compositeConfig.getInt(
                ...
                DEFAULT_REFRESH_PERIOD), webPort, webPort);
    }
...
//                PROP_MONITOR_EVICT_AFTER, DEFAULT_EVICT_AFTER), compositeConfig.getInt(PROP_MONITOR_REFRESH_PERIOD, DEFAULT_REFRESH_PERIOD), webPort);
//    }
    ...
    public static int getPort() {
        baseInitialized();
        return configuration.compositeConfig.getInt(PROP_PORT, DEFAULT_MASTER_PORT);
    }
    ...
    public static JettyWebServerInit getJettyWebServerInit() {
        ...
        if (configuration.jettyWebServerInit == null) {
            JettyWebServerConfiguration config = new JettyWebServerConfiguration(
                    getIp(), getWebPort(),
                    configuration.webRequestTimeout, configuration.webThreads, getHomepage());

            configuration.jettyWebServerInit = new JettyWebServerInit(config);
        }
    ...
    public static Address getDaemonAddress() {
        baseInitialized();
        if (configuration.peer0Address == null) {
            configuration.peer0Address = new Address(getIp(), getPort(), getDaemonId());
        }
    ...
    public static void printConfigurationValues() {
        ...
        logger.info("Local Address (Peer0Address):\t {}", getDaemonAddress());
...
//        logger.info("Monitor Server:\t\t{}", getMonitorConfiguration().getMonitorServerAddress());
        logger.info("Web Server:\t\t\t{}", getJettyWebServerInit().getConfiguration().getHomePage());
        logger.info("");
        logger.info("");
        logger.info("");
        logger.info("");
