...
public class PLab  {
	...
	public static void setTestObj(PLab testObj) {
	...
	public static class TestPLabComponent extends ComponentDefinition {
		...
		public TestPLabComponent() {
			...
			trigger(new PLabInit(cred,PlanetLabConfiguration.getPlcApiAddress()),pLabComponent.getControl());
		...
		public Handler<SshConnectResponse> handleSshConnectResponse = new Handler<SshConnectResponse>() {
			public void handle(SshConnectResponse event) {
				...
				System.out.println("Connected to: " + event.getHost());
...
//	@org.junit.Test
	...
	public void testPLab()
	{
		...
		try {
			...
			Kompics.createAndStart(PLab.TestPLabComponent.class, 2);
		...
		try {
			PLab.semaphore.acquire(EVENT_COUNT);
