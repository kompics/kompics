...
public class ScpTester {
	...
	public static void setTestObj(ScpTester testObj) {
	...
	public static class TestSshComponent extends ComponentDefinition {
		...
		private static ScpTester testObj = null;
		...
		public TestSshComponent() {
			...
			connect(sshComponent.getNegative(DownloadUploadPort.class), downloadUploadMgComponent
					.getPositive(DownloadUploadPort.class));

			connect(downloadUploadMgComponent.getNegative(ScpPort.class), scpComponent
		...
		public Handler<Start> handleStart = new Handler<Start>() {
			public void handle(Start event) {
				...
				trigger(new SshConnectRequest(cred, timerId, host), sshComponent
		...
		public Handler<SshConnectResponse> handleSshConnectResponse = new Handler<SshConnectResponse>() {
			public void handle(SshConnectResponse event) {
				...
				if (UPLOAD == false) {
					DownloadFileRequest command = new DownloadFileRequest(UUID.randomUUID(),
							event.getSessionId(),
							FILE_TO_COPY, FILE_TARGET_DIR, "", SshComponent.HIERARCHY, 10 * 1000,
							true);
					trigger(command, sshComponent.getPositive(SshPort.class));
				} else {
					UploadFileRequest command2 = new UploadFileRequest(UUID.randomUUID(), event.getSessionId(),
							new File(FILE_TO_COPY), FILE_TARGET_DIR, false, 300 * 1000.0, true);
					trigger(command2, sshComponent.getPositive(SshPort.class));
				}
			}
		};

		public Handler<DownloadFileResponse> handleDownloadFileResponse = new Handler<DownloadFileResponse>() {
			public void handle(DownloadFileResponse event) {
				...
				UploadFileRequest command = new UploadFileRequest(UUID.randomUUID(),
						event.getSessionId(), new File(
						FILE_TO_COPY), FILE_TARGET_DIR, true, 10 * 1000, true);
				trigger(command, sshComponent.getPositive(SshPort.class));
		...
		public Handler<UploadFileResponse> handleUploadFileResponse = new Handler<UploadFileResponse>() {
			public void handle(UploadFileResponse event) {
				...
				trigger(new HaltRequest(event.getSessionId()), sshComponent
						.getPositive(SshPort.class));
			}
		};
	...
	public ScpTester() {
		 ...
		 * 
		 * // session.execCommand("ls /bin"); session.startShell(); //
		 ...
		 * 
		 * Thread.sleep(200); while (stdout.available() > 0) { byte[] response =
		 * new byte[stdout.available()];
		 * 
		 * stdout.read(response, 0, response.length); System.out.println(new
		 * String(response)); Thread.sleep(200); } int cond =
		 * session.waitForCondition(ChannelCondition.EOF, 5000);
		 ...
		 * 
		 * session.close(); sshConn.close();
		 * 
		 * }
		 * 
		 * } catch (SocketTimeoutException e) {
		 * System.err.println("connection timeout: " + e.getMessage()); } catch
...
//	@org.junit.Test
	...
	public void testSsh() {
		...
		try {
			...
			Kompics.createAndStart(ScpTester.TestSshComponent.class, 1);
		...
		try {
			ScpTester.semaphore.acquire(EVENT_COUNT);
