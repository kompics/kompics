 ...
 * subsequent to the access or removal of that element from the {@code
...
public class ConcurrentLinkedQueue<E> extends AbstractQueue<E> implements
	...
	private static class Node<E> {
		...
		void recycle(E val, Node<E> n) {
			itemUpdater.set(this, val);
			nextUpdater.set(this, n);
		}
	}

	@SuppressWarnings("unchecked")
	private Node<E> acquire(E x, Node<E> next) {
		// Node<E> node = (Node<E>) free.poll();
		// if (node != null) {
		// node.recycle(x, next);
		// return node;
		// } else {
		return new Node<E>(x, next);
		// }
	}
	...
	private boolean casHead(Node<E> cmp, Node<E> val) {
		boolean removedHead = headUpdater.compareAndSet(this, cmp, val);
		if (removedHead) {
			if (cmp != null) {
				release(cmp);
			}
