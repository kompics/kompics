...
/**
 * The <code>CyclonPeer</code> class represents the peer component for Cyclon.
 * It contains the Cyclon component, which implements the Cyclon protocol, as
 * well as a bootstrap client component, a monitor client component, and a web
 ...
 * plus a <code>TChord</code> component, implementing the T-Chord protocol and a
 * <code>Chord</code> component initialized with the set of Chord neighbors
 * computed by T-Chord. The <code>TChordPeer</code> component should implement
 * the following flow of events: (1) bootstrap (like this component), (2) ask
 * the Cyclon component to join a Cyclon overlay, (3) wait 15 Cyclon cycles so
 * that Cyclon gets random view, (4) start TChord (which uses Cyclon for
 * initialization), (5) when TChord signals termination, use the set of Chord
 * neighbors output by TChord to initialize and start the Chord component.
 * 
...
public final class CyclonPeer extends ComponentDefinition {
	...
	public CyclonPeer() {
		...
		monitor = create(CyclonMonitorClient.class);
		webapp = create(CyclonWebApplication.class);
	...
	Handler<CyclonPeerInit> handleInit = new Handler<CyclonPeerInit>() {
		public void handle(CyclonPeerInit init) {
			self = init.getSelf();
			...
			trigger(new CyclonInit(init.getConfiguration()), cyclon
					.getControl());
			trigger(new BootstrapClientInit(self, init
					.getBootstrapConfiguration()), bootstrap.getControl());
			trigger(new CyclonMonitorClientInit(init.getMonitorConfiguration(),
	...
	Handler<JoinCyclon> handleJoin = new Handler<JoinCyclon>() {
		public void handle(JoinCyclon event) {
			...
			trigger(new CyclonWebApplicationInit(cyclonSelf,
