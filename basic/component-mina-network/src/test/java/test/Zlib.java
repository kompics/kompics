 ...
 * as a compressor and decompressor, but only as one at a time. The only flush
 * method supported is <tt>Z_SYNC_FLUSH</tt> also known as
...
class Zlib {
	 ...
	 *            the level of compression that should be used. One of
	 *            <tt>COMPRESSION_MAX</tt>, <tt>COMPRESSION_MIN</tt>,
	 *            <tt>COMPRESSION_NONE</tt> or <tt>COMPRESSION_DEFAULT</tt>
	 * @param mode
	 *            the mode in which the instance will operate. Can be either of
	 *            <tt>MODE_DEFLATER</tt> or <tt>MODE_INFLATER</tt>
	 ...
	 * 
	 * @param inBuffer
	 *            the {@link IoBuffer} to be decompressed. The contents of the
	 *            buffer are transferred into a local byte array and the buffer
	 *            is flipped and returned intact.
	 * @return the decompressed data
	 * @throws IOException
	 *             if the decompression of the data failed for some reason.
	 * @throws IllegalArgumentException
	 *             if the mode is not <code>MODE_DEFLATER</code>
	 */
	public IoBuffer inflate(IoBuffer inBuffer) throws IOException {
		...
		// We could probably do this better, if we're willing to return multiple
		// buffers
	 ...
	 * 
	 * @param inBuffer
	 *            the buffer to be compressed. The contents are transferred into
	 *            a local byte array and the buffer is flipped and returned
	 *            intact.
